...
SELECT 
  ...
  -- Trending score based on upvotes, comments, and recency
  (p.upvotes * 2 + (SELECT COUNT(*) FROM public.comments WHERE post_id = p.id) * 1.5 + 
   CASE 
     WHEN p.created_at > now() - interval '1 day' THEN 10
     WHEN p.created_at > now() - interval '3 days' THEN 5
...
CREATE INDEX IF NOT EXISTS idx_posts_trending_score ON public.posts USING btree ((upvotes * 2 + created_at::date - current_date));
